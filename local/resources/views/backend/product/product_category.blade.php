...
@section('conten')
            ...
            <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-4">
                <div class="">
                    <button class="btn btn-primary shadow-md mr-2" data-tw-toggle="modal"
                        data-tw-target="#add_product_category" onclick="product_category_add()">เพิ่ม
            ...
            <div class="card-block" style="margin-top:10px;">
                <div class="row">
                    <div class="dt-responsive table-responsive">
                        <table id="table_product_category" style="width:100%;"
    ...
    <div id="add_product_category" class="modal" data-tw-backdrop="static" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog ">
            <div class="modal-content">
                {{ Form::open(['url' => ['/admin/product_category/store'], 'id' => 'productCate-upload', 'autocomplete' => 'off', 'enctype' => 'multipart/form-data']) }}
                <!-- BEGIN: Modal Header -->
                <div class="modal-header">
                    <h2 class="font-medium text-base mr-auto">เพิ่ม หมวดหมู่สินค้า</h2>
                ...
                <div class="modal-body grid grid-cols-12 gap-4 gap-y-3 bg-slate-100/50">
                    <div class="col-span-12">
                        <div>
                            <label for="regular-form-1" class="form-label">Category Name :
                                <span class="text-danger name_err _err"></span>
                            </label>
                            <input id="regular-form-1" name="category_name" id="category_name" type="text"
                    ...
                    <div class="col-span-6">
                        <div>
                            <select type="text" class="rounded" name="select_pro_cate_lang" id="select_pro_cate_lang"
    ...
    @include('backend.product.product_category_edit')
@endsection
...
@section('script')
    ...
    <script>
        ...
        function product_category_add() {
            $('#productCate-upload').attr('action', "{!! url('/admin/product_category/store') !!}");
            $('#submit').text('Save');
            // $('#add_product_category').modal('show');
        }

        function editProduct_Category(id) {
            $(`input[name="id"]`).val(id);
            $('#product_category-edit').attr('action', "{!! url('/admin/product_category/edit') !!}");
            $.ajax({
                type: 'GET',
                url: '{{ url('/admin/product_category/edit_data') }}',
                data: {
                    id: id
                },
                success: function(result) {
                    console.log(result);
                    $('input[name^=category_name_update').val(result['category_name'])
                    $('#select_pro_cate_lang_update').val(result['lang_id'])
        ...
        function del_user(id) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: "GET",
                        url: "{!! url('/admin/product_category/delete/" + id + "') !!}",
