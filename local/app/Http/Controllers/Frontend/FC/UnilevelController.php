...
class UnilevelController extends Controller
{
    ...
    public static function uplineAB($introduce_id)
    {
        ...
        foreach ($members as $member) {
            ...
            $data = UnilevelController::check_type_register($introduce_id, 1);
            ...
            while ($data['status'] == 'fail' && $data['code'] == 'run') {
                ...
                $i++;
                $data = UnilevelController::check_type_register($data['arr_user_name'], $i);
            }
    ...
    private static function check_recursive_auto_plac($user_name, $lv)
    {
        ...
        foreach ($data_check as $value) {
            $checked++;
            $check = UnilevelController::check_auto_plack($value->user_name);
