...
class JPController extends Controller
{
    ...
    public function jp_clarify()
    {
        ...
        if (Auth::guard('c_user')->user()->qualification_id == 'MC') {
            ...
            $rate = 0;
        } elseif (Auth::guard('c_user')->user()->qualification_id == 'MB') {

            $rate = 80;
        } elseif (Auth::guard('c_user')->user()->qualification_id == 'MO') {

            $rate = 90;
        } elseif (Auth::guard('c_user')->user()->qualification_id == 'VIP') {
        ...
        $pv_to_price = 1 * $rate / 100;
        $data = ['pv_to_price' => $pv_to_price, 'rs' => $data, 'rate' => $rate];
    ...
    public function jang_pv_cash_back(Request $rs)
    {
        ...
        if ($rs->type == 2) {
            ...
            $customer_update->bonus_total =  $customer_update->bonus_total + $pv_to_price_tax;
    ...
    public function jang_pv_active(Request $rs)
    {
        ...
        if ($wallet_g->qualification_id == 'MC') {

            $rate = 0;
        } elseif ($wallet_g->qualification_id == 'MB') {

            $rate = 80;
        } elseif ($wallet_g->qualification_id == 'MO') {

            $rate = 90;
        } elseif ($wallet_g->qualification_id == 'VIP') {
                ...
                foreach ($report_bonus_active as $value) {
                    ...
                    if ($value->bonus > 0) {
                        ...
                        DB::table('report_bonus_active')
                            ->where('id', $value->id)
                            ->update([
                                'ewalet_old' => $wallet_g_user,
                                'ewalet_new' => $wallet_g_total,
                                'ewallet_use_old' => $ewallet_use,
                                'ewallet_use_new' => $ewallet_use_total,
                                'status' => 'success',
                                'date_active' => now()
                            ]);
                    }
    ...
    public function jang_pv_upgrad(Request $rs)
    {
        ...
        if ($data_user->qualification_id == 'MC') {
            if ($pv_upgrad_total >= 30 and $pv_upgrad_total < 1000) { //อัพ MO
                if ($rs->pv_upgrad_input >=  1) {
                    if (empty($data_user->expire_date)) {
                        // ถ้าไม่มีวันหมดอายุ ให้เริ่มนับจากวันนี้ +33 วัน
                ...
                if ($rs->pv_upgrad_input >=  30 and $rs->pv_upgrad_input < 200) {
                    if (empty($data_user->expire_date)) {
                        // ถ้าไม่มีวันหมดอายุ ให้เริ่มนับจากวันนี้ +33 วัน
                        $expire_date = date('Y-m-d', strtotime('+33 day'));
                    } else {
                    ...
                    if (empty($data_user->expire_date_bonus)) {
                        ...
                        if ($days_diff < 33) {
                            // ถ้าวันคงเหลือน้อยกว่า 33 วัน ให้บวกเพิ่มให้ครบ 33 วัน
                            $days_to_add = 33 - $days_diff;
                            $expire_date_bonus = date('Y-m-d', strtotime("+{$days_to_add} day", $expire_time));
                        } else {
                            // ถ้ามากกว่าหรือเท่ากับ 33 วัน ไม่ต้องเปลี่ยนแปลง
                            $expire_date_bonus = $data_user->expire_date_bonus;
                        }
                    ...
                    if (empty($run_data_user->name)) {
                        ...
                        if ($i == 1) {
                            ...
                            if ($qualification_id == 'MC') {
                                ...
                                $wallet_total =  round($rs->pv_upgrad_input * $rate / 100, 3);
                            ...
                            if ($qualification_id == 'MC' || $qualification_id == 'MB') {
                                ...
                                $wallet_total = round($rs->pv_upgrad_input * 10 / 100, 3);
                                $arr_user[$i]['bonus'] = $wallet_total;
                                $report_bonus_register[$i]['tax_total'] = round($wallet_total * 3 / 100, 3);
                                $report_bonus_register[$i]['bonus_full'] = $wallet_total;
                                $report_bonus_register[$i]['bonus'] = round($wallet_total - $wallet_total * 3 / 100, 3);
                            ...
                            if ($qualification_id == 'MC' || $qualification_id == 'MB' || $qualification_id == 'MO') {
                                ...
                                $wallet_total =  round($rs->pv_upgrad_input * 5 / 100, 3);
                                $arr_user[$i]['bonus'] = $wallet_total;
                                $report_bonus_register[$i]['tax_total'] = round($wallet_total * 3 / 100, 3);
                                $report_bonus_register[$i]['bonus_full'] = $wallet_total;
                                $report_bonus_register[$i]['bonus'] =  round($wallet_total - $wallet_total * 3 / 100, 3);
