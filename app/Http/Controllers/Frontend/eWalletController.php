...
class eWalletController extends Controller
{
    ...
    public function transfer(Request $request)
    {
        ...
        if (floatval($customer_transfer->ewallet_use)  >= 200 || floatval($customer_transfer->ewallet_tranfer)  >= 200) {

            if ($customer_transfer->ewallet >= $request->amt) {


                if ($customer_transfer->ewallet_use >= 200) {
                    $ewallet_use =   floatval($customer_transfer->ewallet_use) -   floatval($request->amt);

                    if ($ewallet_use < 0) {
                        $customer_transfer->ewallet_use = 0;
                        $ewallet_tranfer =  floatval($customer_transfer->ewallet_tranfer) +  $ewallet_use;
                    ...
                    $ewallet_tranfer =   floatval($customer_transfer->ewallet_tranfer)  -  floatval($request->amt);
                    if ($ewallet_tranfer < 0) {
                ...
                $ewallet =  floatval($customer_transfer->ewallet) -  floatval($request->amt);
                ...
                $customer_receive->ewallet =  floatval($customer_receive->ewallet)  +  floatval($request->amt);
                $customer_receive->ewallet_tranfer =  floatval($customer_receive->ewallet_tranfer)  +  floatval($request->amt);
    ...
    public function withdraw(Request $request)
    {
        ...
        try {
            ...
            // ===== คำนวณยอดที่ถอนได้ =====
            $ewallet_use     = floatval($customer->ewallet_use);
            $ewallet_tranfer = floatval($customer->ewallet_tranfer);
            $ewallet_total   = floatval($customer->ewallet);
